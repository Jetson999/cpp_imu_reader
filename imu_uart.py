...
ser_port = "COM4"     #This needs to be replaced with the corresponding serial port number. For Windows systems, it is written as COMx. If it is Linux, it needs to be adjusted according to the system used, such as /dev/ttyUSBx or /dev/ttySx.
...
def Cmd_RxUnpack(buf, DLen):
    ...
    if buf[0] == 0x11:
        ...
        if ((ctl & 0x0004) != 0):
            ...
            # 验证滤波效果：计算陀螺仪数据变化率
            gyro_change = abs(tmpX - last_gyro_data[0]) + abs(tmpY - last_gyro_data[1]) + abs(tmpZ - last_gyro_data[2])
            last_gyro_data = [tmpX, tmpY, tmpZ]
...
def Cmd_GetPkt(byte):
    ...
    if RxIndex == 0: # Start code
        ...
        if byte == CmdPacket_End: # 捕Get the complete package
            ...
            if frame_count % 50 == 0:  # 每50帧打印一次统计信息
                actual_fps = frame_count / elapsed if elapsed > 0 else 0
                print(f"\n[帧率验证] 已接收 {frame_count} 帧, 耗时 {elapsed:.2f}秒, 实际帧率: {actual_fps:.2f} Hz\n")
...
def read_data():
    global frame_count, start_time, last_gyro_data
    print("------------demo start--------------")
    ...
    print("=" * 50)
    print("参数配置验证:")
    print(f"  数据传输帧率 (params[5]): {params[5]} Hz")
    if params[5] == 0:
        print(f"    实际帧率: 0.5 Hz")
    else:
        print(f"    实际帧率: {params[5]} Hz")
    print(f"  陀螺仪滤波系数 (params[6]): {params[6]}")
    print("=" * 50)
